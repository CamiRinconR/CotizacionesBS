<x-layout.layout title="Usuarios">
</x-layout.layout>
<header class="px-6 py-4 space-y-2 text-center">
<h1 class="font-serif text-3xl text-sky-600 dark:text-sky-500">Usuarios</h1>
    ...
    @foreach($user as $use)
        <div class="max-w-3xl px-4 py-2 space-y-4 bg-white rounded shadow dark:bg-slate-800">
            <h2 class="text-xl text-center text-slate-600 dark:text-slate-300 hover:underline">
                <a href="{{route('usuarios.show',$use)}}">
                {{$use->name}}</a>
            </h2>
            ...
            <a class="inline-flex items-center text-xs font-semibold tracking-widest text-center uppercase transition duration-150 ease-in-out dark:text-slate-400 text-slate-500 hover:text-slate-600 dark:hover:text-slate-500 focus:outline-none focus:border-slate-200"
               href="{{route('usuarios.edit',$use)}}">Editar usuario</a>
            <form action="{{route('usuarios.destroy', $use)}}" method="POST">
                ...
                <button class="inline-flex items-center text-xs font-semibold tracking-widest text-center text-red-500 uppercase transition duration-150 ease-in-out dark:text-red-500/80 hover:text-red-600 focus:outline-none"
                        type="submit">Eliminar usuario</button>
            </form>
@endforeach
